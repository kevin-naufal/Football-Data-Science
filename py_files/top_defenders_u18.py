"""
Analisis Top 10 Pemain Defender dengan Potensi Tertinggi di Bawah Umur 18
"""
...
def analyze_top_defenders_u18():
    """
    Analisis top 10 pemain defender dengan potensi tertinggi di bawah umur 18
    """
    print("="*60)
    print("TOP 10 DEFENDER BERUMUR DI BAWAH 18 - POTENSI TERTINGGI")
    print("="*60)
    ...
    try:
        ...
        df = pd.read_csv(os.path.join(base_dir, 'csv_files', 'fifa_players.csv'), low_memory=False)
        print(f"\n[OK] Data berhasil dimuat: {len(df)} pemain\n")
        
        # Filter pemain umur di bawah 18
        df_u18 = df[df['age'] < 18].copy()
        print(f"[INFO] Pemain di bawah umur 18: {len(df_u18)} pemain")
        
        # Filter defender - posisi yang berhubungan dengan defender
        defender_positions = ['CB', 'LB', 'RB', 'LWB', 'RWB', 'CDM']
        df_defenders = df_u18[df_u18['player_positions'].str.contains('|'.join(defender_positions), na=False)]
        print(f"[INFO] Defender di bawah umur 18: {len(df_defenders)} pemain\n")
        
        # Analisis potensi
        if 'potential' in df.columns:
            # Ambil top 10 pemain dengan potential tertinggi
            top_defenders = df_defenders.nlargest(10, 'potential')
            
            # Pilih kolom yang relevan
            result = top_defenders[['short_name', 'club_name', 'nationality_name', 'player_positions', 
                                   'potential', 'overall', 'defending', 'age']]
            
            print("Rank | Nama Pemain           | Klub                 | Negara      | Posisi   | Potential | Defлending | Umur")
            print("-" * 110)
            ...
            for idx, (_, row) in enumerate(result.iterrows(), 1):
                ...
                pos = str(row['player_positions'])[:9] if len(str(row['player_positions'])) > 9 else str(row['player_positions'])
                
                defending = row['defending'] if pd.notna(row['defending']) else 0
                
                print(f"{idx:2d}.  | {name:20s} | {club:20s} | {nation:11s} | {pos:9s} | {row['potential']:9.0f} | {defending:10.0f} | {row['age']:4.0f}")
            
            print("\n" + "="*60)
            print("DETAIL PEMAIN - DEFENDER MUDA BERBINANG")
            print("="*60)
            
            for idx, (_, row) in enumerate(result.iterrows(), 1):
                print(f"\n{idx}. {row['short_name']} خرید(Umur: {int(row['age'])} tahun)")
                print(f"   Klub: {row['club_name']}")
                print(f"   Negara: {row['nationality_name']}")
                print(f"   Posisi: {row['player_positions']}")
            ...
            ax1 = axes[0]
            colors = plt.cm.YlOrRd(range(len(result)))
            ...
            ax1.set_title('Top 10 Defender Muda (Umur < 18) - Potential Tertinggi', fontsize=14, fontweight='bold')
            ax1.invert_yaxis()
            ...
            ax2.bar([i - width/2 for i in x], result['potential'], width, 
                   label='Potential', color='darkred')
            ax2.bar([i + width/2 for i in x], result['overall'], width, 
                   label='Overall Saat Ini', color='darkblue')
            ax2.set_xlabel('Defender Muda (Umur < 18 tahun)')
            ax2.set_ylabel('Rating')
            ax2.set_title('Perbandingan Potential vs Overall Rating', fontweight='bold')
            ax2.set_xticks(x)
            ax2.set_xticklabels([f"{name}\n({age})" for name, age in zip(result['short_name'], result['age'])], 
                               rotation=45, ha='right')
            ax2.legend()
            ax2.grid(axis='y', alpha=0.3)
            
            plt.tight_layout()
            plt.savefig(os.path.join(base_dir, 'png_files', 'top_10_defenders_u18.png'), dpi=300, bbox_inches='tight')
            print("[OK] Visualisasi disimpan sebagai 'top_10_defenders_u18.png'")
            plt.show()
            
            # Simpan ke CSV
            result.to_csv(os.path.join(base_dir, 'csv_files', 'top_10_defenders_u18.csv'), index=False)
            print("[OK] Data disimpan sebagai 'top_10_defenders_u18.csv'")
            ...
            for pos in defender_positions:
                count = result['player_positions'].str.contains(pos, na=False).sum()
                if count > 0:
                    print(f"{pos}: {count} pemain")
